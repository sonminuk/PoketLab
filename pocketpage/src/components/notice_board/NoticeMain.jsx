...
const NoticeMain = () => {
  ...
  const navigateToPostDetail = (postId) => {
    navigate(`/post/${postId}`, { state: user ? { uid: user.uid, email: user.email } : {} });
  };
    ...
    <div className="container">
      ...
      {user ? (
        <>
          ...
          <button onClick={navigateToPostForm} className="writeButton">게시글 작성</button>
          <button onClick={navigateToMyPage} className="myPageLink">마이페이지</button>
        </>
      ) : (
      ...
      <div className="boardNavigation">
        {['공지사항', '정보', '질문', '일반'].map((board) => (
          <button
            ...
            className={currentBoard === board ? 'activeBoardButton' : 'boardButton'}
          >
            {board}
          </button>
        ))}
      ...
      <PostList user={user} navigateToPostDetail={navigateToPostDetail} board={currentBoard} />
    </div>
  );
};
